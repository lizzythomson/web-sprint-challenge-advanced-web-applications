...
A token is used to verify that the user is allowed to visit certain pages/sites. The token can be saved/used after a user logins in for example. It often is seen as a key that ensures that the user is authorizorized.

2. What steps can you take in your web apps to keep your data secure?

- Use private routes and redirects that only allows authenicated users access to certain information or pages
- Only allow the token to be valid for a certain amount of time before deleting and causing the user to relogin
- If the user leaves your application, cause the user to be automatically logout
- Ensure that users have secure passwords (to avoid easy hacking)

3. Describe how web servers work.

A web server is a computer that runs the websites. The web server stores the "information". When the user causes a HTTP request the web server responses to that request whether to create/read/update/delete and delivers the response back to the user.
